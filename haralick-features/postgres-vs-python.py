...
plt.bar_label(rects_0, fmt='%.1f', fontsize=16)
rects_1 = plt.bar(index + 0.12, bar_data[0], bar_width, bottom=y_offset, color=colors[0], label=rows[0])
plt.bar_label(rects_1, fmt='%.1f', fontsize=16)
plt.legend(loc='upper center', ncols=2, fontsize=16)

plt.ylabel(f"Уменьшение времени исполнения, кол-во раз", fontdict={'fontsize': 20})
